...
async function getHubs() {
    const hubs = await getTreeData('/hubs');
    return hubs.map(hub => createTreeNode(`hub|${hub.id}`, hub.attributes.name, 'icon-hub', true));
}

async function getProjects(hubId) {
    const projects = await getTreeData(`/hubs/${hubId}/projects`);
...
async function getContents(hubId, projectId, folderId = null) {
    const contents = await getTreeData(`/hubs/${hubId}/projects/${projectId}/contents` + (folderId ? `?folder_id=${folderId}` : ''));
...
async function getVersions(hubId, projectId, itemId) {
    const versions = await getTreeData(`/hubs/${hubId}/projects/${projectId}/contents/${itemId}/versions`);
